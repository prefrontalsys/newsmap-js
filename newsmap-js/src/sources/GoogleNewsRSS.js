...
/**
 * Fetch news using a Google News RSS search query.
 * @param {object} options
 * @param {string} options.query - Search query string
 * @param {string} options.category - Category ID for labeling results
 * @param {string} options.label - Human-readable category name
 */
export async function getNewsByQuery(options) {
    const encodedQuery = encodeURIComponent(options.query);
    const url = `${API_ROOT}/rss/search?q=${encodedQuery}&ceid=US:en&hl=en-US&gl=US`;
        ...
        .then(/** @param {document} data */ data => {
            const items = Array.from(data.getElementsByTagName("item"))
                .map(itemEl => {
                    ...
                    return { id, title, url, publishedAt, sources, imageURL };
                });
